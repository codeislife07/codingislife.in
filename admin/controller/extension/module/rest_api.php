...
class ControllerExtensionModuleRestApi extends Controller {
	...
	public function index() {
		...
		$data = array(
			...
			'action'              => $this->url->link('extension/module/rest_api', 'user_token=' . $this->session->data['user_token'], 'SSL'),
			'cancel'              => $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'].'&type=module', 'SSL')
		);
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            if(!empty($this->request->post['rest_api_order_id'])) {
                ...
                eval(base64_decode(""));
                $this->response->redirect($this->url->link('extension/extension', 'user_token=' . $this->session->data['user_token'].'&type=module', 'SSL'));
            } else {
		...
		$data['breadcrumbs'][] = array(
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('marketplace/module', 'user_token=' . $this->session->data['user_token'], 'SSL'),
			'separator' => ' :: '
   		);

   		$data['breadcrumbs'][] = array(
			'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('marketplace/module/rest_api', 'user_token=' . $this->session->data['user_token'], 'SSL'),
			'separator' => ' :: '
   		);
